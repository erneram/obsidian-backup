# Changes — stage-2 (CSS fix + provisioning + módulos por rol)

## P1 — Modal Permisos ancla arriba

- `RoleListPage.vue` — `DialogContent` permisos: `top-[6vh] translate-y-0 max-h-[88vh] overflow-hidden flex flex-col`
- `RoleListPage.vue` — `TabsList`: `flex-nowrap`; `TabsTrigger`: `shrink-0` (scroll horizontal)

## P2A — Provisioning al crear tenant

- `domain/tenant/mod.rs` — `TenantForCreate` + campo `use_template: bool` (default `true`)
- `repositories/tenant_repository.rs::create` — reemplaza seed parcial con tx atómica:
  clona roles + role_permissions + menu_item_roles de tenant `default` cuando `use_template=true`;
  aborta con error claro si `default` no existe
- `migrations/032_backfill_tenant_provisioning.sql` — backfill idempotente para tenants huérfanos
- `TenantListPage.vue` — checkbox `useTemplate` (default `true`) en form de crear
- `adminService.ts` — `TenantCreate.useTemplate?: boolean`
- `locales/es.json` — `admin.tenants.useTemplate` + `useTemplateHint`

## P2B — Asignación de módulos por rol

- `domain/rbac/menu_item_role.rs` — `set_items_for_role(role_id, ids, tenant_id)` en trait
- `repositories/menu_item_role_repository.rs` — impl: DELETE + INSERT dentro de tx
- `web/dto/rbac.rs` — `SetMenuItemsRequest { menu_item_ids: Vec<Uuid> }`
- `web/handlers/rbac.rs` — `get_role_menu_items` + `set_role_menu_items` (guard subconjunto)
- `web/router.rs` — `GET/PUT /api/roles/{id}/menu-items` en grupo `rbac_tenant` (TenantAdmin+)
- `RoleListPage.vue` — botón "Módulos" por fila + modal con checkboxes; auto-include padres
- `adminService.ts` — `getRoleMenuItems(id)` + `setRoleMenuItems(id, ids)`
